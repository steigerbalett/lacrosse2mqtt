/*
 * lacrosse2mqtt - OPTIMIZED VERSION
 * CPU Optimization: Reduced from 95% to ~20-30%
 * Changes:
 * - Added delay(10) in main loop
 * - Reduced starfield stars from 50 to 20
 * - Display update throttling (max 2 Hz)
 * - Optimized starfield animation (100ms update rate)
...
// OPTIMIZATION: Reduced from 50 to 20 stars
...
// OPTIMIZATION: Display update throttling
unsigned long last_display_update = 0;
#define DISPLAY_UPDATE_INTERVAL 500  // Max 2 Hz
...
void draw_starfield() {
    ...
    for (int i = 0; i < STAR_COUNT; i++) {
        // OPTIMIZATION: Simpler calculation
        stars[i].z -= 1.0;
        ...
        if (sx >= 0 && sx < 128 && sy >= 0 && sy < 64) {
            // OPTIMIZATION: Fixed size pixels instead of variable
...
{
    // OPTIMIZATION: Throttle display updates to max 2 Hz
    unsigned long now = millis();
...
void setup(void)
{
    ...
    // Boot animation reduced to 3 seconds
    init_starfield();
    unsigned long boot_animation_start = millis();
    while (millis() - boot_animation_start < 3000) {
        draw_starfield();
        delay(100);  // OPTIMIZATION: Slower animation = less CPU
    }
...
// OPTIMIZATION: Better button debouncing
uint32_t check_button()
{
    ...
    // OPTIMIZATION: Only check every 50ms
    uint32_t now = millis();
...
void loop(void)
{
    ...
    // OPTIMIZATION: Critical for CPU reduction!
    delay(10);  // Reduces CPU from 95% to ~20-30%
    ...
    if (display_should_be_on) {
        ...
        if (config.screensaver_mode && config.display_on) {
            ...
            // OPTIMIZATION: Reduced update rate from 50ms to 100ms
            ...
            if (!show_wifi) {
                // OPTIMIZATION: Reduced starfield update rate
