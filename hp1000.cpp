...
bool TryHandleData(uint8_t *data, int len, Frame *frame) {
    // HP1000 Frame Format (18 bytes mit UV/Lux):
    // [0] = Preamble (0xFF)
    // [1] = Sensor ID
    ...
    // [11-12] = Pressure (0.1 hPa, offset 300)
    // [13] = Rain (0.3mm per tip)
    // [14] = UV Index (0-15)
    // [15-16] = Light (Lux, 16-bit)
    // [17] = CRC8
    ...
    // CRC Check
    if (crc8(data, 17) != data[17]) return false;
    ...
    // Luftdruck
    uint16_t pressure_raw = (data[11] << 8) | data[12];
    frame->pressure = (pressure_raw / 10.0) + 300.0;
    
    // Regen
    frame->rain = data[13] * 0.3;
    
    // UV-Index
    frame->uv = data[14];
    
    // Lichtintensität (Lux)
    uint16_t light_raw = (data[15] << 8) | data[16];
