...
/*
 * TX35IT Temperature/Humidity Sensor Protocol
 * Data rate: 9579 bps
 * Similar to LaCrosse but different data rate
 */

byte TX35IT::CalculateCRC(byte *data, byte len)
{
    byte crc = 0;
    for (int i = 0; i < len; i++) {
...
bool TX35IT::DecodeFrame(byte *bytes, struct Frame *f)
{
    f->valid = true;
...
bool TX35IT::TryHandleData(byte *data, struct Frame *f)
{
    if ((data[0] & 0xF0) == 0x90) {
...
bool TX35IT::DisplayFrame(byte *data, struct Frame *f)
{
    static unsigned long last[SENSOR_NUM];
    if (!f->valid) {
        Serial.println("TX35IT::DisplayFrame FRAME INVALID");
        return false;
    }

    DisplayRaw(last[f->ID], "TX35IT  ", data, FRAME_LENGTH, f->rssi, f->rate);
...
void TX35IT::DisplayRaw(unsigned long &last, const char *dev, uint8_t *data, uint8_t len, int8_t rssi, int rate)
{
